...
public class MusicDao {
	...
	private static MusicDao mMusicDao = null;
	/**
	 * 单例
	 * @return httpUtils单例
	 */
	...
	private MusicOpenHelper musicOpenHelper;// 数据库帮助类对象

	public MusicDao(Context context) {
		musicOpenHelper = new MusicOpenHelper(context);
	}
	...
	public void insert(MusicInfo musicInfo) {
		...
		if (db.isOpen()) {
			db.execSQL(
					"insert into musicinfo(mMusicId,song_id,album_id, duration, title,artist, album, filedata) values(?,?,?,?,?,?,?,?);",
	...
	public void deleteItem(MusicInfo info){
		SQLiteDatabase db = musicOpenHelper.getWritableDatabase();
		if (db.isOpen()) {
			db.execSQL("delete from musicinfo where song_id = ?;",
	...
	public void UpdateMusicInfo(String title,String artist,String album,int music_id) {
		SQLiteDatabase db = musicOpenHelper.getWritableDatabase();
		if (db.isOpen()) {
			db.execSQL("update musicinfo set title = ? ,artist = ? ,album = ? where mMusicId = ?;",new Object[]{title,artist,album,music_id});
			db.close();
		}
	}
	...
	public void ChangeMusicId(int fromIndex,int repeatTimes){
		...
		if (db.isOpen()) {
			for(int i= 0;i<repeatTimes;i++){
				db.execSQL("update musicinfo set mMusicId = ? where mMusicId = ?;",new Object[]{fromIndex,fromIndex+1});
				fromIndex+=1;
			}
			db.close();
		}
	...
	public ArrayList<MusicInfo> queryAll() {
		...
		if(db.isOpen()) {
			Cursor cursor = db.rawQuery("select mMusicId,song_id,album_id, duration, title,artist, album, filedata from musicinfo;", null);
	...
	public MusicInfo queryItem(int son_id) {
		SQLiteDatabase db = musicOpenHelper.getReadableDatabase();	// 获得一个只读的数据库对象
		if(db.isOpen()) {
			Cursor cursor = db.rawQuery("select mMusicId,song_id,album_id, duration, title,artist, album, filedata from musicinfo where song_id = ?;", new String[]{son_id + ""});
